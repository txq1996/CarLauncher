...
object Store {
    ...
    data class V2Button(
        ...
        /**
         * 判断两个按钮在业务上是否"完全相同"——含分屏左右顺序，
         * 即 `left` 与 `right` 也参与比较。
         * 用于底栏添加/替换时检测重复快捷方式。
         */
        fun sameAs(o: V2Button): Boolean =
            type == o.type && action == o.action
                && id == o.id && left == o.left && right == o.right
    }
    ...
    /** 启动入口 Intent：按应用标识 pkg/cls 显式启动 */
    private fun entryIntent(c: Context, id: String): Intent? {
        val (pkg, cls) = splitId(id)
    ...
    fun launchSplit(c: Context, leftId: String, rightId: String) {
        ...
        if (sent) {
            Handler(Looper.getMainLooper()).postDelayed({ resizeDocked(c) }, 2000)
        }
    }
